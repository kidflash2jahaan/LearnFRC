 ...
 * lf_vid, never both, so this endpoint cannot be used to link an account to its
 * pre-signup browsing. See src/lib/funnel.ts and the funnel_events migration for
 * the rules this is the front door to.
 *
 * Callers do not need to dedupe. Every step is once per subject for all time,
 * enforced by a unique index, so firing on every lesson page view is fine: the
 * second and later hits are no-ops.
 *
 * Server code should call recordFunnelEvent() directly instead of posting here.
 * A Server Action already knows the user id, and skipping the HTTP hop skips the
 * rate limit and the session round trip. This route exists for the steps only
 * the browser can see: opening a lesson, attempting a quiz.
 */
export async function POST(req: Request) {
  ...
  // The step allow-list is the ONLY thing validated off the wire, and it is also
  // a CHECK constraint in the database. An unknown step is rejected here rather
  // than becoming a seventh, invisible bucket nobody ever charts.
  ...
  // Work out the subject SERVER side. The client never says who it is: a beacon
  // body claiming a user id would let anyone forge another account's funnel.
  // sendBeacon sends same-origin cookies, so the session is here if the reader
  // has one. Same shape as /api/presence.
  ...
  try {
    ...
    // Auth unreachable. Fall back to the anonymous key rather than lose the
    // event: an event misattributed as anonymous is recoverable, a dropped one
    // is not.
    userId = null;
  }
