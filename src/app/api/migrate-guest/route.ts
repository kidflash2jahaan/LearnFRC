...
/**
 * Carries a guest's lesson completions into the account they just made: insert
 ...
 *
 * It does not trust the request body. It migrates the rows the SERVER holds in
 * guest_progress for this visitor id, and those only get there through
 * /api/guest-progress's quiz gate. The old version inserted whatever lesson ids
 * the caller posted, so any signed-in user could post all 394 ids and mint 394
 * completions plus the XP and leaderboard rank that come with them, going around
 * the quiz check setLessonComplete enforces.
 *
 * It does not touch profiles.xp. The on_lesson_completed trigger already awards
 * XP on every lesson_progress insert. The old version added another 10 per
 * lesson on top, so every migration paid out roughly double and inflated the
 * leaderboard. Letting the trigger own XP is also what makes the "+10 XP per
 * lesson" line in the signup ask literally true.
 */

/**
 * Mirrors awardAchievements() in src/app/actions/progress.ts. A migrated learner
 * should hold exactly the badges their lessons earned, otherwise the first thing
 * they see after signing up is an empty achievements panel next to a full
 * progress bar.
 */
async function awardAchievements(admin: SupabaseClient, userId: string) {
  ...
  // Current streak, the same definition the progress action uses.
  ...
  // Lesson to department, so fully finished departments can be counted.
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // The body is optional. visitorId is the only thing read out of it.
  }
  ...
  // destabilising the guest flow today.
  const visitorId =
  ...
  // The server's record of what this visitor completed as a guest, oldest first
  // so the streak the XP trigger computes walks forward the way it did when they
  // earned it. Paged, because PostgREST truncates at 1000 rows silently.
  ...
  // Do not double-insert lessons the account already has.
  ...
  const toInsert = guest
    ...
    .map((g) => ({
      ...
      // Keep the date they actually earned it. Stamping everything now() would
      // collapse a fortnight of learning into one day and erase the streak the
      // trigger reads back out of these rows.
  ...
  if (toInsert.length) {
    ...
    // XP is trigger computed by on_lesson_completed, so read back what it
    // awarded rather than adding a second, conflicting amount here.
  ...
  // The guest rows are redundant now, the account owns this progress.
