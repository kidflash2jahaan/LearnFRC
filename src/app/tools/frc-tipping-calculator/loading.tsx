 ...
 *   inputs are a grid gap-4 sm:grid-cols-2 of Field blocks
    ...
    <div className="relative mx-auto max-w-6xl px-4 pb-20 pt-28 sm:px-6 lg:px-8 lg:pt-32">
      <div className="ac-card rounded-2xl p-5 sm:p-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
          {/* INPUTS
           *
           * showAdvanced starts false, so the drive-fraction / CoG-offset
           * block does not render. First paint is four geometry Fields in the
           * sm:grid-cols-2 grid, the amber CoG callout, the friction heading,
           * the tread-preset select with its two helper paragraphs, a lone
           * half-width µ override, and the two ac-btn-ghost buttons — ten
           * vertical rows, not a tidy 2×4 field matrix.
           */}
          <div className="rounded-2xl border border-border bg-white/60 p-5">
            ...
            {/* Track · Wheelbase · CoG height · Weight */}
            ...
            {/* amber "CoG height dominates accuracy" callout */}
            <Skeleton className="mt-3 h-[66px] rounded-xl" />

            <div className="ac-divider my-5" />

            {/* Wheel-on-carpet friction heading + Estimate pill */}
            <Skeleton className="mb-3 h-5 w-64 max-w-full rounded" />

            {/* Wheel / tread preset select + its two helper paragraphs */}
            ...
            {/* µ override — one child of a sm:grid-cols-2 grid, so half width */}
            ...
            {/* Show advanced · Reset to defaults — ac-btn-ghost, min-h-11 */}
            <Skeleton className="mt-5 h-11 w-72 max-w-full rounded-2xl" />
          ...
          <div className="rounded-2xl border border-border bg-white/60 p-5">
            ...
            <Skeleton className="mt-1 h-12 w-40 rounded-lg" />
            <Skeleton className="mt-1 h-5 w-48 rounded" />
            {/* SSF + longitudinal tip tiles */}
            <div className="mt-4 grid grid-cols-2 gap-3">
              {Array.from({ length: 2 }).map((_, i) => (
                <Skeleton key={i} className="h-28 rounded-xl" />
              ))}
            </div>
            {/* slide-vs-tip verdict box */}
            <Skeleton className="mt-4 h-24 rounded-xl" />
            {/* ramp / cross-slope tiles */}
            <div className="mt-4 grid grid-cols-2 gap-3">
              {Array.from({ length: 2 }).map((_, i) => (
